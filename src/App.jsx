  ...
  return (
    <div style={{minHeight:'100vh',padding:'12px 16px'}}>
          ...
          <div className="flex gap-3 flex-1 justify-center flex-wrap">
            {teams.map(t=>(
              <TeamCard key={t.id} team={t}
                active={ansTeamId===t.id||winTeamId===t.id}
                onClick={()=>phase==='question'&&setAnsTeamId(t.id)}
                onDelta={handleDelta}/>
            ))}
          </div>
      ...
      <div className="text-center text-white/30 text-xs mt-3 pb-2">
        Mở Túi Câu Hỏi → Chọn nhóm trả lời → Đúng/Sai → Mở Túi Phần Thưởng
      </div>
